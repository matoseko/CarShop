    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                if payment_option == 'S':
                    return redirect('viewer:payment', payment_option='stripe')
                elif payment_option == 'P':
                    return redirect('viewer:payment', payment_option='paypal')
                else:
                    messages.warning(self.request, "Invalid Payment option")
                    return redirect('viewer:checkout')

        except ObjectDoesNotExist:
            messages.error(self.request, "You do not have an order")
            return redirect("viewer:order-summary")
...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__pk=item.pk).exists():
            ...
            return redirect("viewer:product", pk=pk)
    else:
...
class CarbrandListView(ListView):
    ...
    def get_queryset(self):
        content = {
            ...
            'items': Item.objects.filter(brand__name=self.kwargs['brand'])
        }
        return content
...
def search_product(request):
    ...
    if request.method == "GET":
        ...
        if query_name:
            results = Item.objects.filter(Q(item_name__icontains=query_name)|Q(fuel__name__icontains=query_name)|Q(transmission__name__icontains=query_name)|Q(brand__name__icontains=query_name)|Q(color__icontains=query_name))
            #results | =
            return render(request, 'home.html', {"item_list":results})
